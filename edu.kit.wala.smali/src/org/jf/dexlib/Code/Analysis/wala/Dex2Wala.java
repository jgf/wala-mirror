...
public class Dex2Wala {
	...
	private final ClassDataItem.EncodedMethod method;
	private final List<AnalyzedInstruction> dalvikInstructions;
	private final SymbolTable symbols;
	
	
	private List<SSAInstruction> instructions;
	
	public Dex2Wala(final ClassDataItem.EncodedMethod m) {
		method = m;
        
		final MethodAnalyzer analyzer = new MethodAnalyzer(method, false,null);
        analyzer.analyze();
        dalvikInstructions = analyzer.getInstructions();

		instructions = new LinkedList<SSAInstruction>();
		symbols = new SymbolTable(method.method.getPrototype().getParameterRegisterCount());
	}
	
	public List<SSAInstruction> build() {
		final String name = method.method.getVirtualMethodString();
	...
	private void run(String name) {
		...
		for(AnalyzedInstruction inst : dalvikInstructions) {
			...
			case CHECK_CAST:
				...
				this.instructions.add(ssa.CheckCastInstruction(inst.getInstructionIndex(), inst.getDestinationRegister(), inst.getParameterRegister(0), trf,true));
				break;
			case CMP_LONG:
...
//			case INVOKE_DIRECT_EMPTY:
			...
			case INVOKE_INTERFACE:
				...
				this.instructions.add(ssa.InvokeInstruction(inst.getInstructionIndex(), params, symbols.newSymbol(),cr));
				break;
			case INVOKE_STATIC:
				mi = (MethodIdItem) ((InstructionWithReference) inst.getInstruction()).getReferencedItem();
				mr = MethodReference.findOrCreate(ClassLoaderReference.Application, mi.getContainingClass().getTypeDescriptor(),
